<template>
  <div>
    <div class="map" :class="{'full-size': isFullSize}">
      <div class="full-size-btn" @click="isFullSize = !isFullSize">toggle full size</div>
      <div class="app-house"
           v-for="house of houses"
           :key="house.id"
           :style="housePosition(house)">
        <img :src="getImgUrl(house.flag)">
      </div>
    </div>
  </div>
</template>

<style lang="scss">
.app-house {
  width: 50px;
  height: 50px;

  img {
    width: 100%;
  }
}
</style>


<script lang="ts">
import {Component, Prop, Vue, Watch} from 'vue-property-decorator';
import House from '@/models/House';

@Component({
  name: 'world-map'
})
export default class WorldMap extends Vue {
  @Prop() houses!: House[];
  isFullSize?: boolean = false;

  housePosition(house: House) {
    const ratio = this.isFullSize ? 2.5 : 1;

    return {
      position: 'absolute',
      left: house.position.x * ratio + 'px',
      top: house.position.y * ratio + 'px',
      'z-index': this.$store.getters.winner && this.$store.getters.winner.name === house.name ? 100 : 1
    }
  }

  getImgUrl(img: string) {
    return require('../assets/' + img)
  }
}
</script>

<style scoped lang="scss">
.map {
  position: relative;
  top: 0;
  right: 0;
  background-image: url('../assets/map.jpg');
  background-size: cover;
  width: calc(1280px * 1.115);
  height: calc(853px * 1.115);
  border-left: 2px solid #55442b;
  box-shadow: 0 2px 4px 0 rgba(0, 0, 0, 0.50);

  &.full-size {
    width: 3600px;
    height: 2400px;
  }

  .full-size-btn {
    position: absolute;
    top: 0;
    left: 0;
    color: #a58e64;
    background-color: #775f3c;
    padding: 3px;
  }
}
</style>
